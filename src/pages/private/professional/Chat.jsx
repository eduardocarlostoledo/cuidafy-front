...
const Chat = ({ id }) => {
  ...
  const handleSendMessage = () => {
    ...
    sendMessage(currentMessage, usuario?.nombre || 'Profesional');
    setCurrentMessage('');
    emitStopTyping();
  };
  ...
  return (
    <div className="p-4 h-1/2 flex flex-col">
      ...
      <div className="flex-1 overflow-auto rounded border border-gray-300 bg-gray-50 p-4 mb-4">
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => (
            <div
              key={index}
              className={`mb-3 ${
                message.senderType === 'profesional' ? 'text-right' : 'text-left'
              }`}
            >
              <div
                className={`inline-block max-w-xs rounded-lg px-3 py-2 ${
                  message.senderType === 'profesional'
