...
import { getGenericServiceImage } from "../data/especialidadImagenes";

const Card = ({ image, text, count, link, countLabel = "horarios visibles" }) => {
  const [currentImage, setCurrentImage] = useState(image || getGenericServiceImage(text));

  useEffect(() => {
    setCurrentImage(image || getGenericServiceImage(text));
  }, [image, text]);
    ...
    <Link to={link}>
      <div className="p-4 h-max bg-white max-width-card max-w-sm 2xl:max-w-sm border rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition duration-300 hover:scale-105 cursor-pointer">
        <LazyLoadImage
          ...
          width={300}
          height={300}
          onError={() => setCurrentImage(getGenericServiceImage(text))}
        />
